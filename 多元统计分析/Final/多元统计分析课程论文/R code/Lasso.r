...
# ==== 预测 ====
pred_prob <- predict(cvfit, newx = x.test.mat, s = "lambda.min", type = "response")
clust.test <- ifelse(pred_prob > 0.5, 1, 0)

# ==== 误分类率 ====
missclassrate <- mean(clust.test != y.test)
cat(sprintf("测试集误分类率为: %.4f\n", missclassrate))

# ==== F1-score ====
TP <- sum(clust.test == 1 & y.test == 1)
FP <- sum(clust.test == 1 & y.test == 0)
FN <- sum(clust.test == 0 & y.test == 1)
